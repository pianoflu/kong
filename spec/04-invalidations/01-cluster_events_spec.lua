...
describe("cluster_events with db: " .. kong_conf.database, function()
  ...
  describe("#o new()", function()
    ...
    it("instanciates only once (singleton)", function()
      finally(function()
        _G.ngx.config.debug = true
      ...
      assert.has_error(function()
        assert(kong_cluster_events.new { dao = dao })
      end, "kong.cluster_events was already instanciated", nil, true)
    end)
    ...
    it("instanciates but does not start polling", function()
      local cluster_events = assert(kong_cluster_events.new { dao = dao })
  ...
  describe("pub/sub", function()
    ...
    it("broadcasts on a given channel", function()
      ...
      -- on a cluster
      local cluster_events_1 = assert(kong_cluster_events.new(kong_conf, dao, {
        node_id = uuid_1
      }))
      local cluster_events_2 = assert(kong_cluster_events.new(kong_conf, dao, {
        node_id = uuid_2
      }))
    ...
    it("runs callbacks in protected mode", function()
      ...
      assert(cluster_events_1:subscribe("errors_channel", function() error("foo") end, false))
    ...
    it("broadcasts an event with a 'nbf' (not before) field", function()
      ...
      assert(cluster_events_1:subscribe("nbf_channel", cb, false))
      ...
      ngx.sleep(0.001) -- still delay in case our timer is set to 0
